...
async function downloadAttachmentFile(id, filename) {
  ...
  try {
    showToast({ title: "Tải file", message: "Đang chuẩn bị tải file...", variant: "info" });
...
async function handleDeleteAttachment(attachmentId, bugId) {
  if (!confirm("Bạn có chắc chắn muốn xóa file đính kèm này?")) return;
  try {
    await deleteBugAttachment(attachmentId);
    showToast({ title: "Xóa file", message: "Đã xóa file đính kèm thành công.", variant: "success" });
        ...
        : attachments.map(a => `
            <div class="d-flex align-items-center justify-content-between p-2 mb-1 app-card rounded-3">
              ...
              <div class="d-flex gap-1">
                <button class="btn btn-sm btn-soft" onclick="downloadAttachmentFile(${a.id}, '${escapeHtml(a.name)}')" title="Tải về"><i class="bi bi-download"></i></button>
  ...
  try {
    ...
    tbody.innerHTML = members.map(m => `
      <tr>
        ...
        <td>
          ${canManage && String(m.role).toLowerCase() !== 'manager'
            ? `<select class="form-select form-select-sm dev-role-select" data-user-id="${m.userId}">
                <option value="developer" ${String(m.role).toLowerCase() === 'developer' ? 'selected' : ''}>Developer</option>
                <option value="tester" ${String(m.role).toLowerCase() === 'tester' ? 'selected' : ''}>Tester</option>
              </select>`
...
function enforceManagerWorkspaceCreation() {
  const user = getUser();
  const isManager = user && String(user.role).toLowerCase() === "manager";
  
  if (!isManager) {
    const createWorkspaceButtons = document.querySelectorAll('[data-bs-target="#createWorkspaceModal"]');
    createWorkspaceButtons.forEach(btn => {
...
async function init() {
  renderUser();
  enforceManagerWorkspaceCreation();
