...
public class WorkspaceServiceImpl implements WorkspaceService {
    ...
    private void checkManagerRole(Integer workspaceId, Integer userId) {
        ...
        boolean isSystemManager = member != null && "Manager".equalsIgnoreCase(member.getRole());
        if (isSystemManager) {
            return;
        }
        ...
        if (!isManager) {
            throw new ForbiddenException("Chỉ Manager mới có quyền thực hiện hành động này");
        }
    }
    ...
    public List<WorkspaceDto> getMyWorkspaces(Integer userId) {
        ...
        if (isSystemManager) {
            ...
            return allWorkspaces.stream()
                    .map(ws -> {
                        WorkspaceDto dto = toDto(ws, userId);
                        dto.setMyRole("Manager");
                        return dto;
                    })
    ...
    public WorkspaceDto createWorkspace(CreateWorkspaceReq req, Integer creatorUserId) {
        Member member = memberRepository.findByUserId(creatorUserId).orElse(null);
        boolean isSystemManager = member != null && "Manager".equalsIgnoreCase(member.getRole());
        if (!isSystemManager) {
            throw new ForbiddenException("Chỉ người dùng có vai trò Manager trên hệ thống mới được phép tạo Workspace.");
        }
    ...
    public WorkspaceMemberDto addMember(Integer workspaceId, AddMemberReq req, Integer requesterId) {
        ...
        if ("Manager".equalsIgnoreCase(req.getRole())) {
            throw new ForbiddenException("Không thể gán vai trò Manager cho thành viên khác");
        }
    ...
    public void updateMemberRole(Integer workspaceId, Integer userId, String newRole, Integer requesterId) {
        ...
        if ("Manager".equalsIgnoreCase(newRole)) {
            throw new ForbiddenException("Không thể cập nhật vai trò của thành viên khác thành Manager");
        }
